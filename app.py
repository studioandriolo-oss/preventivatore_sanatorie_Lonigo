...
def genera_pdf():
    ...
    pdf.cell(0, 10, "1. DATI E STATO DI FATTO", ln=True)
    pdf.set_font("Arial", '', 11)
    pdf.cell(0, 8, f"Tipo Pratica Ipotizzata: {titolo}", ln=True)
    pdf.cell(0, 8, f"Superficie Immobile: {superficie} Mq", ln=True)
    pdf.cell(0, 8, f"Unita' Coinvolte: {unita}", ln=True)
    pdf.ln(5)
    
    # Riepilogo Economico
    pdf.set_font("Arial", 'B', 12)
    pdf.cell(0, 10, "2. RIEPILOGO ECONOMICO (Stima)", ln=True)
    pdf.set_font("Arial", '', 11)
    pdf.cell(100, 8, "Imponibile Professionale:", border=0)
    pdf.cell(0, 8, f"{tot_imponibile:,.2f} Euro", ln=True, align='R')
    
    pdf.cell(100, 8, "Spese Esenti (Art. 15 / Diritti):", border=0)
    pdf.cell(0, 8, f"{tot_art15:,.2f} Euro", ln=True, align='R')
    
    pdf.cell(100, 8, "IVA e Cassa Architetti:", border=0)
    pdf.cell(0, 8, f"{(iva+cassa):,.2f} Euro", ln=True, align='R')
    
    pdf.set_font("Arial", 'B', 11)
    pdf.cell(100, 8, "Totale Spese Tecniche:", border=0)
    pdf.cell(0, 8, f"{tot_tecnico_lordo:,.2f} Euro", ln=True, align='R')
    pdf.ln(2)
    
    pdf.set_font("Arial", '', 11)
    pdf.cell(100, 8, "Stima Sanzione Comune (Oblazione F24):", border=0)
    pdf.cell(0, 8, f"{sanzione:,.2f} Euro", ln=True, align='R')
    pdf.line(10, pdf.get_y(), 200, pdf.get_y())
    pdf.ln(2)
    
    pdf.set_font("Arial", 'B', 14)
    pdf.cell(100, 10, "COSTO TOTALE 'CHIAVI IN MANO':", border=0)
    pdf.cell(0, 10, f"{totale_chiavi_in_mano:,.2f} Euro", ln=True, align='R')
    pdf.ln(10)
    
    # Note a piè di pagina
    pdf.set_font("Arial", 'I', 9)
    note_text = "Rilievo dello stato di fatto eseguito con strumentazione laser scanner 3D SLAM ad alta fedelta' per garantire una restituzione grafica con precisione millimetrica. E' previsto un acconto all'accettazione del preventivo formale."
    pdf.multi_cell(0, 5, note_text)
    
    # Ritorna il PDF in formato binario corretto per il download
